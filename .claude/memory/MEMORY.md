...
- Shot harness needs a well-formed vault: journal at `Journal/<year>/<date>.md`, db entries need `type: db` + `db: <slug>` frontmatter. Run: `flutter run -d macos --dart-define=DASH_SHOT_DIR=<out> --dart-define=DASH_SHOT_VAULT=<vault>`.
